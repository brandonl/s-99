...
object ListsModule {
  ...
  def compress[T]( xs: List[T] ): List[T] =
    ( xs :\ List[T]() ) { (hd, tl) => if( tl.isEmpty || hd != tl.head ) hd::tl else tl }
  ...
  def combinations[T]( n: Int, xs: List[T] ): List[List[T]] = xs match {
    case _ if n == 0 => List(Nil)
    case Nil => Nil
    case y :: ys => ( combinations( n - 1, ys ) map { y :: _ } ) ::: combinations( n , ys )
  }

  def group[T]( sizes: List[Int], xs: List[T] ): List[List[List[T]]] = sizes match {
    case Nil => List(Nil)
    case y :: ys => combinations( y, xs ) flatMap { z =>
        group( ys, ( xs.toSet -- z ).toList ) map { z :: _ }
      }
  }

  def lsort[T]( xs: List[List[T]] ): List[List[T]] = xs sortBy (_.size)

  def lsortFreq[T]( xs: List[List[T]] ): List[List[T]] = {
    val max = ( 0 /: xs ){ (c, e) => if( e.size > c ) e.size else c }
    val st = Array.fill(max)( Nil.asInstanceOf[List[List[T]]] )
    xs foreach { x =>
      val idx = x.size - 1
      st update ( idx, x :: st( idx ) )
    }
    val zt = ( st sortBy { s => s.size } ).flatten
    zt.toList
  }
}
